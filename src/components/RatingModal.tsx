  ...
  return (
    <div onClick={close} style={{ position: 'fixed', inset: 0, background: 'rgba(17,17,17,0.45)', zIndex: 100, display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
      <div onClick={(e) => e.stopPropagation()} style={{ background: 'var(--bg)', width: '100%', maxWidth: 480, borderTopLeftRadius: 16, borderTopRightRadius: 16, padding: 20 }}>
        <h3 style={{ margin: 0, fontSize: 17 }}>How was your delivery?</h3>
        ...
        <div style={{ display: 'flex', justifyContent: 'center', gap: 8, margin: '18px 0' }}>
          {[1, 2, 3, 4, 5].map((n) => (
            <button key={n} onClick={() => setStars(n)} style={{ background: 'none', border: 'none', cursor: 'pointer', fontSize: 36, lineHeight: 1, color: n <= stars ? 'var(--primary)' : 'var(--line)' }}>★</button>
          ))}
        </div>
        ...
        <button onClick={submit} disabled={stars < 1 || saving} className="rf-btn"
          style={{ width: '100%', marginTop: 12, background: 'var(--ink)', color: '#fff', opacity: stars < 1 || saving ? 0.5 : 1 }}>
          {saving ? 'Submitting…' : 'Submit rating'}
        </button>
        <button onClick={close} className="mono" style={{ display: 'block', margin: '12px auto 0', background: 'none', border: 'none', color: 'var(--ink-2)', cursor: 'pointer', fontSize: 11 }}>NOT NOW</button>
      </div>
    </div>
  );
}
