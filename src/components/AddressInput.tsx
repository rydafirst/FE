...
export function AddressInput({ label, placeholder, onSelect }: {
  ...
  return (
    <div className="rf-card" style={{ marginBottom: 12 }}>
      ...
      <input ref={inputRef} className="rf-input" placeholder={placeholder ?? (ready ? 'Search address…' : 'Loading map…')} />
      {error && <p style={{ color: 'var(--danger)', fontSize: 11, margin: '6px 0 0' }}>{error} — add a Maps key (see docs/MAPS-SETUP.md).</p>}
    </div>
  );
}
