...
export default function ActivityDetailPage() {
  ...
  return (
    <main style={{ padding: 20, maxWidth: 480, margin: '0 auto' }}>
      <button onClick={() => router.push('/activity')} className="mono" style={{ fontSize: 'var(--text-caption)', color: 'var(--ink-2)', background: 'none', border: 'none', cursor: 'pointer', padding: 0 }}>‹ ACTIVITY</button>
      <h1 style={{ fontSize: 'var(--text-heading)', margin: '10px 0 12px', letterSpacing: '-0.02em' }}>Delivery details</h1>

      {!job && <p className="mono" style={{ fontSize: 'var(--text-caption)', color: 'var(--mid)' }}>LOADING…</p>}

      {job && (
        <>
          <div style={{ display: 'flex', justifyContent: 'flex-end', marginBottom: 12 }}>
            <span className="rf-pill" style={{ background: l.color, color: 'var(--on-dark)', fontSize: 'var(--text-caption)' }}>{l.text.toUpperCase()}</span>
          </div>
          ...
          {!isRider && rider && (
            <div className="rf-card" style={{ marginBottom: 12 }}>
              <div className="mono" style={{ fontSize: 'var(--text-caption)', color: 'var(--ink-2)', marginBottom: 8 }}>YOUR RIDER</div>
              <b style={{ fontSize: 'var(--text-body)' }}>{rider.name ?? 'Assigned rider'}{rider.nameVerified ? ' ✓' : ''}</b>
          ...
          <div className="rf-card">
            <div className="mono" style={{ fontSize: 'var(--text-caption)', color: 'var(--ink-2)', marginBottom: 10 }}>NEED HELP WITH THIS DELIVERY?</div>
...
function Row({ label, value, strong }: { label: string; value: string; strong?: boolean }) {
  return (
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, padding: '5px 0' }}>
      ...
      <span className="mono" style={{ fontSize: strong ? 'var(--text-body)' : 'var(--text-small)', fontWeight: strong ? 700 : 400, textAlign: 'right' }}>{value}</span>
    </div>
  );
}
