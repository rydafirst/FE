        ...
        return (
          <a key={r.riderId} href={`/admin/riders/${r.riderId}`} style={{ textDecoration: 'none', color: 'inherit' }}>
            <div className="rf-card" style={{ marginBottom: 10, display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
              <div>
                <div className="mono" style={{ fontSize: 'var(--text-small)' }}>{r.riderId.slice(0, 8)}…</div>
                <div className="mono" style={{ fontSize: 'var(--text-caption)', color: 'var(--mid)', marginTop: 4 }}>{r.track ?? 'NO VEHICLE'}</div>
              </div>
              <span className="rf-pill" style={{ background: s.color, color: 'var(--on-dark)', fontSize: 'var(--text-caption)' }}>{s.text.toUpperCase()} ›</span>
            </div>
          </a>
        );
      })}
